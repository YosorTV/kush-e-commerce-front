...
export const UserSession = ({ cta, session, signOutTitle, sessionLinks = [] }: any) => {
  if (!session) {
  ...
  return (
    <nav className='flex items-center justify-end gap-x-5'>
      <div className='dropdown dropdown-end'>
        <div
          tabIndex={-1}
          className={cn('flex cursor-pointer items-center justify-center gap-5', session.picture ? 'w-8' : 'w-full')}
        >
          {session.picture ? (
            <Image
              src={session.picture}
            ...
            <span className='whitespace-nowrap font-semibold uppercase text-base-200'>{session.name}</span>
          )}
        </div>
